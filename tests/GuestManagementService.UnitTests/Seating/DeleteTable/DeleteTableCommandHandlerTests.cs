...
public sealed class DeleteTableCommandHandlerTests
{
    ...
    public async Task Handle_WhenEventIsMissing_ReturnsEventNotFound()
    {
        ...
        Assert.Equal(DeleteTableStatus.EventNotFound, result.Status);
    }
    ...
    public async Task Handle_WhenTableDoesNotExistInLayout_ReturnsTableNotFound()
    {
        ...
        Assert.Equal(DeleteTableStatus.TableNotFound, result.Status);
    }

    [Fact]
    public async Task Handle_WhenTableExists_RemovesItAndSaves()
    {
        var eventId = Guid.NewGuid();
        var layout = SeatingLayout.Create(Guid.NewGuid(), eventId, TestTenantId, Now);
        ...
        var result = await handler.Handle(Command(eventId, table.Id), CancellationToken.None);

        Assert.Equal(DeleteTableStatus.Deleted, result.Status);
        Assert.Empty(layout.Tables);
        unitOfWork.Verify(work => work.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    private static DeleteTableCommand Command(Guid eventId, Guid tableId)
    {
        return new DeleteTableCommand(eventId, tableId) { CurrentUser = TestUser };
    }

    private static DeleteTableCommandHandler CreateHandler(
